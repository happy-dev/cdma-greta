...
Trait DokelioSearchTrait {
  public static function search($str, $filter) {
    ...
    if (!$str) {
      $query_string = "SELECT code_AF, synth_titre, slug_formation, synth_periode_de_formation, synth_formation_accroche FROM formation $where $filter GROUP BY synth_titre ORDER BY synth_titre";
    }
    else {
