...
Trait DokelioFormationTrait {
  public static function getSessions($code_AF) {
    $buffer = array();
    $query_string = "SELECT code_AF, synth_titre, synth_periode_de_formation, objectif_formation, nom_image, url_video_formation, flag_diplomant, flag_eligible_cpf, flagFOAD, flagHTT, synth_formation_accroche, code_SES, lib_onglet_session, Prerequis, FRM_contenu_formation, methodes_pedagogiques, moyens_pedago, modalites_accueil, reconnaissance_des_acquis, intervenants, codeFORMACODE, codeROME, taux_reussite, taux_reussite_comment, taux_insertion, taux_insertion_comment, taux_satisfaction, taux_satisfaction_comment, periode_session, Public_vise, detail_duree_de_formation, comment_duree, prix, comment_tarif, lieu_de_formation, modalites_pedagogiques, effectif, contact, referent_handicap, rythme_formation, slug_formation, slug_domaine, lib_domaine, url_mcf, contact_mel, modalites_evaluation, code_rncp, url_rncp, RS_numero_fiche, RS_url_fiche, suite_de_parcours, perspectives_prof FROM formation WHERE code_AF='IPAF_". $code_AF ."' GROUP BY code_SES ORDER BY SES_periode_debut";
