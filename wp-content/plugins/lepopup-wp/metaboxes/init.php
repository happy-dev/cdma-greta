...
define( 'CMB_META_BOX_URL', rtrim(WP_PLUGIN_URL . '/' . plugin_basename(dirname(__FILE__)) . '/') );
/**
 * Create meta boxes
 */
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'text':
					...
					break;
				case 'text_small':
				//var_dump(strlen($meta));
					echo '<input class="cmb_text_small" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', (strlen($meta)) ? $meta : $field['std'], '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;
				case 'text_medium':
				...
				case 'select':
					...
					foreach ($field['options'] as $option) {
						echo '<option value="', $option['value'], '"', (!empty($meta) && $meta == $option['value']) ? ' selected="selected"' : '', '>', $option['name'], '</option>';
					}
					echo '</select>';
				...
				case 'checkbox':
					$std = '';
					if(empty($meta) && !is_array($meta) && !empty($field['std'])) $std = ' checked="checked"';

					echo '<input type="checkbox" value="true" name="', $field['id'], '" id="', $field['id'], '"', $meta ? ' checked="checked"' : $std, ' />';
					echo '<span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;
				case 'multicheck':
					$std = '';
					if(isset($field['std']) && !empty($field['std']) && empty($meta)) $std = ' checked="checked"';

					echo '<ul>';
					foreach ( $field['options'] as $value => $name ) {
						// Append `[]` to the name to get multiple values
						// Use in_array() to check whether the current option should be checked
						echo '<li><input type="checkbox" name="', $field['id'], '[]" id="', $field['id'], '" value="', $value, '"', in_array( $value, $meta ) ? ' checked="checked"' : $std, ' /><label>', $name, '</label></li>';
					}
					echo '</ul>';
					echo '<span class="cmb_metabox_description">', $field['desc'], '</span>';					
					break;		
	...
	function save( $post_id)  {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( ($field['type'] == 'textarea') || ($field['type'] == 'textarea_small') ) {
				$new = htmlspecialchars( $new['title'] );
			}
			...
			if ( isset( $field['validate_func']) ) {
				...
				if(get_post_meta($post_id, $name) == array())delete_post_meta($post_id, $name);
				//update_post_meta($post_id, $name, array());
				//if(empty($aNewToAdd))add_post_meta( $post_id, $name, array(), false );
				...
				if(!get_post_meta($post_id, $name))update_post_meta($post_id, $name, array());
			} else {
...
function cmb_styles_inline() { 
	...
	?>	
	<style type="text/css">
		table.cmb_metabox td:first-child{ border-bottom: 1px solid #E9E9E9; }
		table.cmb_metabox th { text-align: center; font-weight:bold; padding:10px 0;}
		table.cmb_metabox td {padding: 9px 0 9px 7px;}
		...
		span.cmb_metabox_description { color: #AAA; font-style: italic; font-size:11px;}
