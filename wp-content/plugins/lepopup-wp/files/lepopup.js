...
;(function ($) {
	...
	globals = {
		lePopup : {
			...
			close : "<div id='lepopup-footer'><div id='lepopup-close'></div></div>"
		},
		loading : "loading",
		visible : "visible",
		...
		setToggle : function (ele, show) {
			...
			if (show) {
				if (!ele.is(":visible")) {
					this.setSkin();
					this.setCloseBut();
					ele.stop().fadeIn(this.options.animSpeed, function(){self.bindResize();});
				}
			} else {
				if (ele.is(":visible"))
					this.unbindResize();
					ele.stop().fadeOut(this.options.animSpeed, function(){self.emptyContent();});
			}
		},
